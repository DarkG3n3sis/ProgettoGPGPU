...
cl_event mark_positive(cl_command_queue que, cl_kernel mark_kernel,
	cl_mem d_scan_ping, cl_mem d_input, int nels, size_t lws)
{
...
cl_event scan_stride(cl_command_queue que, cl_kernel scan_kernel,
	cl_event wait_for, cl_mem dst, cl_mem src, int stride, int nels, size_t lws)
{
	size_t gws = round_mul_up(nels, lws);
...
cl_event findall_final(cl_command_queue que, cl_kernel findall_kernel,
	cl_event wait_for, cl_mem d_output, cl_mem d_flags, cl_mem d_scan,
	int nels, size_t lws)
{
	size_t gws = round_mul_up(nels, lws);
...
{
	if (argc < 3) error("usage: findall <lws> <nels>");
	...
	int num_strides = 0;
	{ int tmp = nels - 1; while (tmp > 0) { num_strides++; tmp >>= 1; } }
	if (num_strides == 0) num_strides = 1;
	...
	cl_mem d_flags = clCreateBuffer(ctx, CL_MEM_READ_WRITE, scan_memsize, NULL, &err);
	...
	cl_event mark_evt = mark_positive(que, mark_kernel, d_scan[0], d_input, nels, lws);

	/* Copia d_scan[0] in d_flags (servirà per findall_gpu) */
	cl_event copy_evt;
	err = clEnqueueCopyBuffer(que, d_scan[0], d_flags, 0, 0, scan_memsize,
		1, &mark_evt, &copy_evt);
	...
	for (int s = 0; s < num_strides; s++) {
		...
		cl_event stride_evt = scan_stride(que, scan_kernel, prev_evt,
			d_scan[dst_buf], d_scan[src_buf], stride, nels, lws);

		/* Rilascia il precedente stride (non copy_evt che e' rilasciato nel cleanup) */
	...
	cl_event scan_done_evt = (last_stride_evt != NULL) ? last_stride_evt : copy_evt;

	/* ------------------------------------------------------------------ */
	/* Leggi il count                                                       */
	/* ------------------------------------------------------------------ */
	cl_event map_evt;
	cl_int *h_scan = clEnqueueMapBuffer(que, d_scan_result, CL_TRUE, CL_MAP_READ,
		0, scan_memsize, 1, &scan_done_evt, &map_evt, &err);
	...
	if (count > 0) {
		...
		cl_event findall_evt = findall_final(que, findall_kernel, scan_done_evt,
			d_output, d_flags, d_scan_result, nels, lws);

		err = clWaitForEvents(1, &findall_evt);
		ocl_check(err, "wait findall");

		cl_event map_out_evt;
		cl_int *h_output = clEnqueueMapBuffer(que, d_output, CL_TRUE, CL_MAP_READ,
			0, out_size, 1, &findall_evt, &map_out_evt, &err);
